...
using BLEMicroPythonSample.Pages;
			...
			else
			{
				...
				await Task.Run(async () =>
				{
					...
					try
					{
						...
						while (!selectedDevice.XBeeDevice.IsConnected && retries > 0)
						{
							retries--;
							try
							{
						...
						MainThread.BeginInvokeOnMainThread(async () =>
						{
							...
							await Application.Current.MainPage.Navigation.PushAsync(new TemperaturePage(selectedDevice));
						});
					}
